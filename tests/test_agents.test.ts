...
describe("computePathLCA", () => {
  ...
  test("handles nested paths correctly", () => {
    const paths = ["/a/b/c/d/e", "/a/b/c/d", "/a/b/c"];
    expect(computePathLCA(paths)).toBe(path.resolve("/a/b/c"));
  });

  test("filters out empty paths", () => {
    const paths = ["/Users/test/project", "", "  ", "/Users/test/project/src"];
...
describe("Reasoning Effort CLI flags", () => {
  ...
  test("claude with reasoningEffort appends --effort flag", () => {
    const cmd = manager["buildCommand"](
      "claude",
      "test prompt",
      "plan",
      "claude-sonnet-4-6",
  ...
  test("null reasoningEffort adds no extra flags for claude", () => {
    const cmdWith = manager["buildCommand"](
      "claude",
      "test prompt",
      "plan",
      "claude-sonnet-4-6",
    ...
    const cmdWithout = manager["buildCommand"](
      "claude",
      "test prompt",
      "plan",
      "claude-sonnet-4-6",
...
describe("splitCommandTemplate", () => {
  ...
  test("double-quoted backslash escape sequences", () => {
    expect(splitCommandTemplate('my-cli "hello \\"world\\""')).toEqual(["my-cli", 'hello "world"']);
  });

  test("extra whitespace between tokens is collapsed", () => {
    expect(splitCommandTemplate("cmd   --flag   {prompt}")).toEqual(["cmd", "--flag", "{prompt}"]);
  });
  ...
  test("compatible codex custom command still injects model and mode flags", () => {
    manager["agentConfigs"] = {
      ...
      codex: {
        ...manager["agentConfigs"].codex,
        command:
          "codex -a never exec --skip-git-repo-check --sandbox danger-full-access '{prompt}' --json"
      }
    };
...
describe("buildReplyCommand", () => {
  ...
  test("builds correct gemini reply command in edit mode", () => {
    const cmd = manager.buildReplyCommand("gemini", "do it", null, "edit", "gemini-3-pro-preview");
  ...
  test("builds correct copilot reply command", () => {
    const cmd = manager.buildReplyCommand("copilot", "what next", null, "plan", "claude-sonnet-4");
  ...
  test("builds correct copilot reply command in edit mode", () => {
    const cmd = manager.buildReplyCommand("copilot", "apply fix", null, "edit", "gpt-5");
