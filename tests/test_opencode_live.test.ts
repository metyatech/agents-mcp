...
describe("OpenCode Live E2E", () => {
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn opencode in edit mode and parse tool calls correctly",
    async () => {
      ...
      expect(
        statusResult.bash_commands.some((cmd) => cmd.includes("echo") && cmd.includes("hello"))
      ).toBe(true);
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn opencode in plan mode",
    async () => {
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn opencode with fast effort level",
    async () => {
