...
describe("Gemini Live E2E", () => {
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn gemini and parse tool calls correctly",
    async () => {
  ...
  (shouldRunLive ? test : test.skip)(
    "should handle comprehensive file operations",
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "gemini-test-"));
      const manager = new AgentManager(50, 10, tempDir);
...
8. Create a new file dir4/newfile.txt with content "new file"`;
  ...
  (shouldRunLive ? test : test.skip)(
    `should work through MCP commands${!geminiAvailable ? ` (skipped: ${geminiPathOrError})` : ""}`,
    async () => {
