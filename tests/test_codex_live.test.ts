...
describe("Codex Live E2E", () => {
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn codex and parse tool calls correctly",
    async () => {
      ...
      const statusResult = await pollUntilComplete(
        manager,
        "test-codex",
      ...
      expect(
        statusResult.bash_commands.some((cmd) => cmd.includes("echo") || cmd.includes(testFile))
      ).toBe(true);
  ...
  (shouldRunLive ? test : test.skip)(
    "should spawn codex-agent and parse tool calls correctly",
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), "codex-test-"));
      const manager = new AgentManager(50, 10, tempDir);
...
8. Create a new file dir4/newfile.txt with content "new file"`;
        ...
        expect(statusResult.bash_commands.every((cmd) => typeof cmd === "string")).toBe(true);
        expect(
          statusResult.bash_commands.some(
            (cmd) => cmd.includes("ls") || cmd.includes("rm") || cmd.includes("mkdir")
          )
        ).toBe(true);
        expect(statusResult.last_messages).toBeDefined();
        expect(Array.isArray(statusResult.last_messages)).toBe(true);
  ...
  (shouldRunLive ? test : test.skip)(
    `should work through MCP commands${!codexAvailable ? ` (skipped: ${codexPathOrError})` : ""}`,
    async () => {
      ...
      const mcpStatusResult = mcpTaskResult.agents.find(
        (a: any) => a.agent_id === spawnResult.agent_id
      );
      ...
      expect(mcpStatusResult.bash_commands.every((cmd: string) => typeof cmd === "string")).toBe(
        true
      );
      expect(
        mcpStatusResult.bash_commands.some(
          (cmd: string) => cmd.includes("echo") || cmd.includes(testFile)
        )
      ).toBe(true);
      ...
      expect(mcpStatusResult.last_messages.every((msg: string) => typeof msg === "string")).toBe(
        true
      );
