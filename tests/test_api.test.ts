...
describe("API Integration Tests", () => {
  ...
  describe("handleSpawn", () => {
    ...
    test("should allow duplicate task names", async () => {
      ...
      const existing = new AgentProcess(
        "agent-existing",
        "dup-task",
        "codex",
        "noop",
  ...
  describe("Full Flow: spawn -> status -> read -> stop", () => {
    test("should handle complete lifecycle (mocked agents)", async () => {
      ...
      console.log(
        "Added: flow-1 (codex, running), flow-2 (cursor, running), flow-3 (gemini, completed)"
      );
  ...
  describe("Multi-Agent Status Polling (Live)", () => {
    const liveEnabled =
      process.env.AGENTS_MCP_RUN_LIVE_TESTS === "1" ||
      process.env.AGENTS_MCP_RUN_LIVE_TESTS === "true";

    (liveEnabled ? test : test.skip)(
      "should spawn claude, codex, and cursor agents with different tasks and poll status until done",
      async () => {
...
5. Verify the move was successful`
        ...
        for (const config of agentConfigs) {
          ...
          try {
            ...
            console.log(
              `Spawned ${config.type} agent: ${result.agent_id} (working in ${config.taskDir})`
            );
          } catch (err: any) {
        ...
        while (!allDone && iteration < maxIterations) {
          ...
          if ("agents" in statusResult) {
            ...
            console.log(
              `Status: running=${statusResult.summary.running}, completed=${statusResult.summary.completed}, failed=${statusResult.summary.failed}, stopped=${statusResult.summary.stopped}`
            );
              ...
              if (agent.status === "running") {
                const lastMsg =
                  agent.last_messages?.length > 0
                    ? agent.last_messages[agent.last_messages.length - 1]
                    : null;
                console.log(
                  `  - ${agent.agent_id} (${agent.agent_type}): ${agent.status}, last_message: ${lastMsg ? lastMsg.substring(0, 50) + "..." : "none"}`
                );
              }
            }
          ...
          for (const agent of finalStatus.agents) {
            const lastMsg =
              agent.last_messages?.length > 0
                ...
                : null;
            console.log(
              `  - ${agent.agent_id} (${agent.agent_type}): ${agent.status}, last_message: ${lastMsg ? lastMsg.substring(0, 100) : "none"}`
            );
          }
        }
    ...
    test("should respect limit parameter", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const agent = new AgentProcess(
          ...
          null,
          AgentStatus.COMPLETED,
          new Date(`2025-01-0${i + 1}T10:00:00Z`),
          new Date(`2025-01-0${i + 1}T10:30:00Z`)
        );
...
`;
    ...
    test("should reject ralph mode without cwd parameter", async () => {
      ...
      let error: any = null;
      try {
        await handleSpawn(manager, "ralph-task", "codex", "Build something", null, "ralph", null);
      } catch (err: any) {
        error = err;
      }
