...
import { AgentManager, AgentProcess, AgentStatus } from "../src/agents.js";
...
describe("Task-Based API", () => {
  ...
  describe("edge cases", () => {
    ...
    test("should truncate long commands in quick status", () => {
      const longCommand =
        "npm run build -- --config=production --verbose --debug --output=/very/long/path/that/exceeds/one/hundred/characters/easily";
      const events = [{ type: "bash", command: longCommand, timestamp: "2024-01-01" }];
